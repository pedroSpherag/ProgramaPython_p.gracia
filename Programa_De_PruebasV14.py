...
def ProbarSelenoide(client,imeis):
    ...
    imeisAveriados=[]
    print("\nPrueba selenoide:\n~~~~~~~~~~~~~~~~~")
    eAutoManu = input("\nPulsa 'm' probar de forma manual [Open->Close], por defecto sera automatica [Synchro] \n> ")
...
def AbrCerrSelenoide(client,imeis):
    global imeix
    eAbrCerr = input("\nPulsa 'a' --> abrir valvula de forma manual, por defecto se mandara orden de cerrar \n> ")
    dataT=obtenerDataT(imeis)
    if eAbrCerr=='a':
        print("\nAbrir selenoide:\n~~~~~~~~~~~~~~~~~~")     
        for x in range (len(imeis)): 
...
def BorraryPonerSombra(client,imeis,soloPonerSombra):
    global imeix
    print("\nBorrar y poner Sombra nueva:\n~~~~~~~~~~~~~~~~~~~~~~~~~~~~")
    print("\nNota: En el caso de existir error de comunicacion, se recomienda continuar hasta el final del programa. ")
...
def DormirXmin(client,imeis):
    tiempo = input("\nPulsa un n� de forma que [se despierta cada n� * 30min], por defecto sera cada 30 min \n> ")
...
def CparaContinuar():
    ...
    while True: #Se espera a que se aprete la tecla c
        ...
        if letra=='c':
            print("ok")
            break 
#------------eliminar imeis con problemas de comunicaci�n
...
def PintaryGuardarResultado(imei,prueba,errCom):
 ...
 #-------------Funcion para borrar pantalla segun su sistema operativo       
def borrarPantalla(): 
    if os.name == "posix":
       os.system ("clear")
    elif os.name == "ce" or os.name == "nt" or os.name == "dos":
       os.system ("cls")
#-----------------Parte inicial (se leen imeis y se muestran para su comprobaci�n)
...
def funcionSM(client,imeis,nombre,orden):
    ...
    for x in range (len(imeis)): 
        ...
        client.publish(sombraUp[x],'{"state":{"reported":{'+orden+'}}}')  #publish (Se cambia el modo de energ�a) 
        time.sleep(1)  
